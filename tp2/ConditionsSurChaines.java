class ConditionsSurChaines extends Program {
    void algorithm(){
        ...
        boolean condBprecedeA = length(a) > 0 && length(b) > 0 && charAt(b, 0) < charAt(a, 0);//À MODIFIER
        // (Optionnel) Condition c est un prefixe de a
        boolean condCprefixeDeA = length(a) >= length(c) && equals(substring(a, 0, length(c)), c);//À MODIFIER
